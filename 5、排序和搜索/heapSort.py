...
# 堆排序步骤
"""
1、构造最大堆: 若数组下标范围为0~n-1，考虑到单独一个元素是大根堆，则从下标(n//2)开始(包括n//2以后的元素都将是大根堆的叶结点)，递增的元素均为大根堆。
于是只要从(n//2-1)最后一个非叶子结点开始，递减向前依次调整构造大根堆，这样就能保证，构造到某个节点时，它的左右子树都已经是大根堆。
...
i为父结点，2*i+1为左结点，2*i+2为右结点（i从0开始）
"""
...
def heapSort(alist):
    ...
    for end in range(length-1, 0, -1):
        ...
        heapify(alist, 0, end-1) # 取end-1是为了排除end位置的结点

# 调整最大堆: 将堆的start位置的结点的子节点作调整，使得子节点永远小于父节点
# 即以start为根结点的堆调整为最大堆
